...
resource "aws_instance" "caliper" {
  ...
  provisioner "remote-exec" {
    inline = [
      "timeout 120 /bin/bash -c 'until stat /var/lib/cloud/instance/boot-finished 2>/dev/null; do echo waiting ...; sleep 1; done'",
      "sh $HOME/append_auth_keys.sh ${join(" ", formatlist("'%s'", var.user_ssh_public_keys))}",
      "sudo apt-get update && sudo apt-get install -y apparmor apt-transport-https ca-certificates curl build-essential git",
      "curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add - ",
      "sudo sh -c 'echo \"deb [arch=amd64] https://download.docker.com/linux/ubuntu bionic stable\" > /etc/apt/sources.list.d/docker.list'",
      "sudo apt-get update && sudo apt-get -y install docker-ce",
      "sudo usermod -aG docker $USER",
      ...
      "sudo chmod +x /usr/local/bin/docker-compose",
      "sleep 30",
      "sudo docker run --rm -d --name rabbitmq -v /home/ubuntu/caliperProject/rabbitmq/enabled_plugins:/etc/rabbitmq/enabled_plugins -p 1883:1883 -p 5672:5672 -p 15672:15672 rabbitmq:3-management",
      "curl -sL https://deb.nodesource.com/setup_10.x | sudo -E bash -",
      "sudo apt-get update && sudo apt-get install -y nodejs",
      "sudo sh $HOME/caliperProject/setup.sh '${var.vpc_name}' '${var.region}'",
      "cd $HOME",
      "git clone ${var.caliper_repo}",
      "cd $HOME/caliper",
      "git checkout ${var.caliper_version}",
      "npm i && npm run repoclean -- --yes && npm run bootstrap",
      "npm i && npm run repoclean -- --yes && npm run bootstrap",
      "cd $HOME/caliperProject",
      "sudo chmod +x *.sh",
      "sleep 30",
    ]
  }
...
resource "aws_route53_record" "caliper_dns" {
  zone_id = "${aws_route53_zone.private.zone_id}"
  name    = "caliper.${var.vpc_name}.${var.region}"
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.caliper.private_ip}"]
}
...
resource "aws_instance" "ibft_bootnode" {
  ...
  tags = {
    Name = "benchmark-besu-${var.vpc_name}-bootnode"
  }
...
resource "aws_instance" "ibft_rpcnode" {
  ...
  tags = {
    Name = "benchmark-besu-${var.vpc_name}-rpcnode"
  }
  ...
  provisioner "file" {
    source = "files/orion/keys/rpcnode/orion.pub"
    destination = "$HOME/besu/orion.pub"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo apt-get update && sudo apt-get install -y apparmor apt-transport-https ca-certificates curl build-essential openjdk-11-jdk python3 python3-setuptools python3-pip python3-dev python3-virtualenv python3-venv virtualenv",
      "[ -e /dev/nvme0n1 ] && sudo mkdir -p /opt/besu/data && sudo mkfs.xfs /dev/nvme0n1 && sudo mount /dev/nvme0n1 /opt/besu/data",
      "sudo sh $HOME/besu/setup.sh '${var.besu_version}' '${var.besu_download_url}' '${aws_instance.ibft_bootnode.private_ip}' '${var.create_orion_nodes ? aws_instance.orion_rpcnode[0].private_ip : "0.0.0.0"}'",
      "sleep 30",
    ]
  }
}
...
resource "aws_instance" "ibft_nodes" {
  ...
  tags = {
    Name = "benchmark-besu-${var.vpc_name}-${count.index}"
  }
...
resource "aws_instance" "orion_bootnode" {
  ...
  provisioner "remote-exec" {
    inline = [
      "timeout 120 /bin/bash -c 'until stat /var/lib/cloud/instance/boot-finished 2>/dev/null; do echo waiting ...; sleep 5; done'",
      "sudo apt-get update && sudo apt-get install -y apparmor apt-transport-https ca-certificates curl build-essential openjdk-11-jdk python3 python3-setuptools python3-pip python3-dev python3-virtualenv python3-venv virtualenv",
      "sudo sh $HOME/orion/setup_bootnode.sh '${var.orion_version}' '${var.orion_download_url}' '${aws_instance.orion_bootnode[0].private_ip}' '${self.private_ip}'",
      "sleep 30",
    ]
  }
}

resource "aws_instance" "orion_rpcnode" {
  count = "${var.create_orion_nodes}" ? 1 : 0
  ami = "${data.aws_ami.ubuntu.id}"
  instance_type = "${var.orion_instance_type}"
  key_name = "${var.default_ssh_key}"
  ...
  tags = {
    Name = "orion-${var.vpc_name}-rpcnode"
  }
  ...
  provisioner "file" {
    source = "files/orion/keys/rpcnode"
