...
for IP in $ALL_NODE_IPS; do
  log_info "Disabling JFR on '${ALL_NODE_PREFIXES_ARR[$I]}'"
  ssh \
    -i "$SSH_KEYFILE" \
    -o "UserKnownHostsFile=/dev/null" \
    -o "StrictHostKeyChecking=no" \
    ubuntu@$IP \
    sudo sed -i "'s/BESU_OPTS.*/BESU_OPTS=\\\"$BESU_OPTS_NOJFR_ESCAPED\\\"/'" /etc/systemd/system/besu.service \&\& sudo systemctl daemon-reload
  I=$(( $I + 1 ))
done
